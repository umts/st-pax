...
RSpec.describe Passenger do
  ...
  describe 'expiration_display' do
    ...
    context 'temporary passenger' do
      it 'returns the expiration date of the eligibility_verification' do
        date = 14.days.from_now
        create :eligibility_verification, passenger: @passenger, expiration_date: date
  ...
  describe 'rides_expire' do
    ...
    context 'eligibility_verification is present' do
      context 'eligibility_verification is expired within grace period' do
        it 'returns 3 days from the eligibility_verification expiry' do
          date = 2.days.ago.to_date
          create :eligibility_verification, passenger: @passenger, expiration_date: date
          expect(@passenger.rides_expire).to eq 3.business_days.since(date)
        end
      end
      context 'the eligibility_verification is not expired' do
        it 'returns three business days after the expiration date of the note' do
          date = 14.days.from_now
          note = create :eligibility_verification, passenger: @passenger, expiration_date: date
          expect(@passenger.rides_expire).to eq 3.business_days.after(note.expiration_date)
        end
      end
    end
    context 'temporary passenger has no eligibility_verification' do
